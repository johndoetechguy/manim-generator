...
DEFAULT_CONFIG = {
    "manim_model": "gemini/gemini-2.0-flash-exp",
    "review_model": "gemini/gemini-2.0-flash-exp",
    "review_cycles": 4,
    "manim_logs": False,
    "streaming": False,
    "temperature": 0.4,
...
class Config:
    ...
    def _create_parser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument(
            "--provider",
            type=str,
            help="Specific provider to use for LiteLLM requests (e.g., 'anthropic', 'openai', 'google')",
        )
    ...
    def _build_config(self, args) -> dict:
        ...
        if not output_dir:
            model_name = args.manim_model.replace("gemini/", "").replace("openrouter/", "").replace("/", "_")
            output_dir = (
