...
```bash
git clone https://github.com/johndoetechguy/manim-generator.git
cd manim-generator
```
...
Then edit `.env` and add your Gemini API key. Get your API key from [Google AI Studio](https://aistudio.google.com/app/apikey).

You can also use other models supported by LiteLLM by setting the appropriate environment variables (OPENAI_API_KEY, ANTHROPIC_API_KEY, etc.).
...
| Argument         | Description                                                                              | Default                           |
| ---------------- | ---------------------------------------------------------------------------------------- | --------------------------------- |
| `--manim-model`  | Model to use for generating Manim code                                                   | "gemini/gemini-2.0-flash-exp"     |
| `--review-model` | Model to use for reviewing code                                                          | "gemini/gemini-2.0-flash-exp"     |
...
| `--provider`     | Specific provider to use for LiteLLM requests (e.g., 'anthropic', 'openai', 'google')   | -                                 |
...
```bash
uv run manim-generate --video-data "Explain the concept of neural networks with visual examples" --review-cycles 3
```

Or with the command directly (if virtual environment is activated):

```bash
manim-generate --video-data "Explain the concept of neural networks with visual examples" --review-cycles 3
```

To use other models (e.g., OpenAI or Anthropic):

```bash
uv run manim-generate --video-data "Your prompt" --manim-model "gpt-4" --review-model "gpt-4"

uv run manim-generate --video-data "microservice architecture" --manim-model "gpt-4" --review-model "gpt-4"
```
...
Images are available only when the reviewer model supports multimodal input. Gemini models support vision by default.

For information about supported models, see:
